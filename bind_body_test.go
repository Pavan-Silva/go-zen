...
func TestBindBody_Empty(t *testing.T) {
	r := New(":0")
	r.Handle("GET /body", func(c *Context) {
...
func TestBind_Body(t *testing.T) {
	...
	r := New(":0")
	r.Handle("POST /bind", func(c *Context) {
		var p payload
...
func TestBind_NoBody(t *testing.T) {
	r := New(":0")
	r.Handle("POST /nobody", func(c *Context) {
...
func TestBindPathParams_NoMatch(t *testing.T) {
	...
	r := New(":0")
	r.Handle("GET /users/{userID}", func(c *Context) {
		var p params
		if err := c.BindPathParams(&p); err != nil {
			c.Error(http.StatusBadRequest, err.Error())
			return
...
func TestBindPathParams_Int(t *testing.T) {
	...
	r := New(":0")
	r.Handle("GET /items/{id}", func(c *Context) {
		var p params
		if err := c.BindPathParams(&p); err != nil {
			c.Error(http.StatusBadRequest, err.Error())
			return
...
func TestBind_WithPathAndQuery(t *testing.T) {
	...
	r := New(":0")
	r.Handle("POST /items/{id}", func(c *Context) {
		var p bindReq
		if err := c.Bind(&p); err != nil {
			c.Error(http.StatusBadRequest, err.Error())
			return
