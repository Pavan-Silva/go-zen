...
func TestRequestID_HeaderConstant(t *testing.T) {
	...
	if got := w.Header().Get(HeaderXRequestID); got == "" {
		t.Fatalf("expected non-empty %q response header", HeaderXRequestID)
	}
}
